"""Synchronise raw Open-Meteo JSON objects from S3 to the pipeline volume."""
...
WEATHER_SITE_FOLDER = os.getenv("WEATHER_SITE_FOLDER", "open_meteo").strip("/")
S3_PREFIX = f"{S3_RAW_PREFIX}/{WEATHER_SITE_FOLDER}"
LOCAL_RAW_BASE = Path(os.getenv("LOCAL_RAW_DIR", "data/raw"))


def list_s3_json_objects(bucket_name: str, prefix: str) -> list[dict]:
...
def is_already_downloaded(local_path: Path, remote_obj: dict) -> bool:
    return (
        local_path.exists()
        and local_path.stat().st_size == remote_obj["Size"]
    )


def is_recent_year_key(key: str, current_year: int | None = None) -> bool:
    """Current and previous-year weather objects may still be revised."""
...
def download_s3_object(
    bucket_name: str, key: str, local_path: Path
) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to download {key} from S3: {error}"
        ) from error
...
def main() -> None:
    ...
    print(
        f"Syncing raw Open-Meteo JSON from S3 bucket '{S3_BUCKET_NAME}' "
        f"with prefix '{S3_PREFIX}'"
    )
    objects = list_s3_json_objects(S3_BUCKET_NAME, S3_PREFIX)
    if not objects:
        raise FileNotFoundError(
            f"No JSON objects found in s3://{S3_BUCKET_NAME}/{S3_PREFIX}"
        )
    ...
    print("Raw Open-Meteo S3 sync completed successfully.")
